...
def _draft_haro_replies(db: Session, digest_text: str) -> list[dict]:
    ...
    system_prompt = (
        ...
        "reply referencing the piece's actual topic for an ordinary single-question query -- longer "
        "and structured, per the many-sub-question guidance above, for a query built around several "
        "numbered questions on one topic; including at least one real URL - from the fixed "
