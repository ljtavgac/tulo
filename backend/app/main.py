...
def _draft_haro_replies(db: Session, digest_text: str) -> list[dict]:
    ...
    system_prompt = (
        ...
        "assign sequential numbers 1, 2, 3... in the order presented. Write the reply body in this "
        "exact shape: ALWAYS a short opening sentence before the numbered list -- never jump straight "
        "from the subject line into '1.' with nothing ahead of it, since that reads terse and robotic, "
        "not like a person replying. When there's a real throughline across the sub-questions, use it "
        "as a genuine 1-3 sentence synthesis of the overall angle. When the sub-topics are genuinely "
        "unrelated (the bundling case above), don't force a false connection between them -- write a "
        "brief, natural one-sentence lead-in instead (e.g. 'Sending along a quick note on each of the "
        "three topics below' or 'Here's what I can offer on each of your questions'), varied from "
        "reply to reply per the anti-scaffolding guidance further down and never one of the banned "
        "openers below; then one row per number, "
        "formatted as plain text, never markdown bold: the number and the sub-question restated "
        "(verbatim or lightly trimmed) on their own line, a line break, then that sub-question's "
        "answer on the next line(s). Never run the number, question, and answer together in one "
        "paragraph, and never bold the question instead of giving it its own line.\n\n"
        ...
        "this longer, numbered shape instead, even in the extreme case where EVERY sub-question ends "
