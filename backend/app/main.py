...
def _draft_haro_replies(db: Session, digest_text: str) -> list[dict]:
    ...
    system_prompt = (
        ...
        "Always skip podcast-guest-booking opportunities ('be a guest on our show', 'find podcasts', "
        "episode/appearance pitches) even when food/cooking-related and even with a real reply email -- "
        "out of scope regardless of topic fit; never draft a reply or propose a content_opportunity for "
        "one of these.\n\n"
        ...
        "A query is often built around more than one distinguishable ask -- either many numbered "
        "sub-questions on one topic (e.g. a dozen numbered questions all about fixing common "
        "homemade-latte mistakes) or several genuinely distinct sub-topics bundled under one "
        "reporter/outlet/deadline with no numbering at all (e.g. one reporter asking, in the same "
        "piece, about wooden cutting boards discoloring, filet mignon vs. tenderloin, AND coconut "
        "flake storage -- three unrelated questions, not one). Treat both the same way: draft exactly "
        "ONE reply covering every sub-question/sub-topic -- never split it into several reply "
        "fragments, and never drop the reply entirely just because none of them turn out to have an "
        "existing match (a real, reported bug: a 3-topic bundled query where every topic needed a new "
        "page ended up with zero reply at all, just orphaned content_opportunity ideas with nothing "
        "tying them together for the reviewer). Don't manufacture this treatment for a single coherent "
        "question that just happens to be phrased with multiple clauses -- only when there are "
        "genuinely separate asks a reporter would accept separate answers to. search_tulo_content "
        "separately for each one (milk steaming, grind size, bean selection, and cheap tools might "
        "each turn up a different page, or none at all).\n\n"
        "Number every sub-question/sub-topic using the reporter's own numbers verbatim when the "
        "source already numbers them (a source's '1.'/'2.'/'3.' stays '1.'/'2.'/'3.'); when the "
        "source doesn't number them (a bundled-but-unnumbered query like the cutting-boards example), "
        "assign sequential numbers 1, 2, 3... in the order presented. Write the reply body as a real "
        "email a person would actually send, not a bare numbered list -- confirmed live, a reply that "
        "jumps straight into '1.' with nothing ahead of it and nothing after the last row reads like a "
        "data dump, not correspondence. Every numbered reply gets this exact shape:\n"
        "  1. A greeting on its own line: 'Hi <reporter_name>,' when reporter_name is known, otherwise "
        "just 'Hi,' -- never skipped.\n"
        "  2. A short context sentence naming what this replies to (the piece, outlet, and/or topic) "
        "and framing that a set of answers/resources follows -- e.g. 'I'm writing in response to your "
        "query on <topic> for <outlet>. Here are a few articles we've written at Tulo that might help "
        "answer your questions below:'. Always phrase this as content Tulo itself authored (\"articles "
        "we've written/produced\"), never as if merely pointing to someone else's site (\"things from "
        "Tulo\" reads as a link round-up, not authorship). When there's a real throughline across the sub-questions beyond just the "
        "topic, fold that genuine synthesis into this sentence or the one after it. When the "
        "sub-topics are genuinely unrelated (the bundling case above), keep this to the plain "
        "context-plus-framing sentence rather than forcing a false connection between them.\n"
        "  3. One row per number, formatted as plain text, never markdown bold: the number and the "
        "sub-question restated (verbatim or lightly trimmed) on their own line, a line break, then "
        "that sub-question's answer on the next line(s). Never run the number, question, and answer "
        "together in one paragraph, and never bold the question instead of giving it its own line.\n"
        "  4. A closing line offering to help with follow-ups (e.g. 'Let me know if you have any "
        "questions or if there's anything else I can help clarify.'), then a sign-off on its own two "
        "lines: 'Thanks for your time,' then 'Tulo Team' -- matching the same closing this pipeline's "
        "own templated follow-ups already use once an article goes live (see _resolve_pending_articles).\n"
        "Vary the exact phrasing of the greeting/context/closing sentences from reply to reply (per "
        "the anti-scaffolding guidance further down) so they don't read like an identical template "
        "pasted into every email, but the STRUCTURE -- greeting, context-plus-framing, numbered list, "
        "closing offer, sign-off -- stays the same every time. This is the one place in this whole "
        "system where a 'Let me know if...' closer and a fixed sign-off are correct, not a banned "
        "AI-tell -- they're what makes it read like a real, complete email instead of a fragment; the "
        "ban on stock openers/closers further down is about the ordinary short single-question reply "
        "below, which has no such structure of its own.\n\n"
        "This reply is a template the reviewer touches up by hand before sending, not finished prose "
        "-- so every sub-question's answer line ends in exactly one of three things, and NEVER an "
        "explanation of which one or why:\n"
        ...
        "sub-question, not the whole bundled/numbered query) -- the two must match verbatim so a "
        "reviewer can pair them up and drop in the real link once that page goes live.\n"
        ...
        "The 3-5 sentence length below is for an ordinary single-question query with a real existing "
        "match; a multi-part query like this -- whether reporter-numbered or bundled -- always earns "
        ...
        "reply referencing the piece's actual topic and including at least one real URL - from the "
        "fixed tools list or a search_tulo_content result, never invented - for an ordinary "
        "single-question query with a real match; longer and numbered, per the guidance above, for a "
        "query built around several sub-questions/sub-topics, whether reporter-numbered or bundled -- "
        "that numbered shape doesn't need a real URL anywhere in it as long as every row ends in a "
        "real URL, a [URL placeholder --...] tag, or a [can't produce a URL for this] tag; use a "
        "single hyphen with spaces around it for a dash, never a double hyphen or em dash). Draft a "
        "\"reply\" for every "
        ...
        "Important distinction: a per-query forwarding alias printed in the digest text -- most "
        "commonly HARO's own 'reply+<uuid>@helpareporter.com' format, one distinct address per query "
        "-- IS a real, capturable reporter_email, not a platform-inbox case; it's a genuine, working "
        "email address that routes to the real reporter, even though it isn't their personal address. "
        "Capture it exactly as printed for the specific query it's paired with in the digest (a "
        "multi-query digest prints a different alias per query -- match each one to its own query, "
        "never reuse one across queries). Only fall back to reporter_email=null when the query truly "
        "prints no address of any kind, just a bare web-form/submission-portal link. "
    ...
    for item in parsed:
        ...
        if item_type == "reply":
            # A numbered multi-part reply is legitimately allowed to have
            # ZERO real URLs when every sub-question needs a new page --
            # see the many-sub-question/bundling prompt guidance above
            # (a real, reported case: a 3-topic bundled query where none
            # of the three had existing coverage still needs its reply
            # drafted, not silently dropped). The URL-or-placeholder
            # check still guards against a reply with nothing real to
            # offer at all -- no real link AND no legitimate pending-
            # article tag means the model produced empty filler, not a
            # genuine template.
            has_real_url = any(url in item["body"] for url in allowed_urls)
            has_placeholder_tag = "[URL placeholder --" in item["body"]
            if not has_real_url and not has_placeholder_tag:
                print(f"  _draft_haro_replies: dropping item with no real Tulo URL and no placeholder tag in body: {label!r}")
                continue
