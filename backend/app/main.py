...
def _draft_haro_replies(db: Session, digest_text: str) -> list[dict]:
    ...
    system_prompt = (
        ...
        "exact shape: a short opening paragraph synthesizing the overall angle (1-3 sentences) ONLY "
        "when there's a real throughline across the sub-questions -- skip it entirely for a bundled "
        "query with no common thread rather than force one; then one row per number, formatted as "
        "plain text, never markdown bold: the number and the sub-question restated (verbatim or "
        "lightly trimmed) on their own line, a line break, then that sub-question's answer on the "
        "next line(s). Never run the number, question, and answer together in one paragraph, and "
        "never bold the question instead of giving it its own line.\n\n"
