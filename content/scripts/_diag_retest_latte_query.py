"""Throwaway: re-ingests the real full HARO latte-mistakes query (the
verbatim 12-question text the user pasted) in isolation through the now
-fixed pipeline (many-sub-question reply guidance), to see whether the
drafted reply now actually addresses more of the 12 items using real
Tulo content instead of one generic mention. Deleted after use."""
...
def main() -> None:
    ...
    payload = {
        ...
        "headers": {"subject": "[RETEST] HARO latte-mistakes query"},
        "plain": QUERY,
        "html": "",
    }
