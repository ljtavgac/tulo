...
def build_request_params(row: dict, collections: list[dict], techniques: list[dict]) -> dict:
    ...
    UNCACHED BY DESIGN, FOR NOW (2026-09-11): STYLE_GUIDE (system, ~500
    tokens) and, within `messages`, the per-template-type `example` block
    plus the `collections`/`techniques` lists are byte-identical across
    every row of the same template_type within one run (collections/
    techniques are fetched once per run, not per row -- see
    build_batch_requests.py's main()), which is a real prompt-caching
    opportunity across a run of a few hundred requests. Deliberately not
    implemented: as of this date it's unclear this pipeline (batch or its
    real-time fallback, submit_realtime_fallback.py -- same request
    bodies, different transport) will even be the mechanism for the next
    content batch (compare: the most recent new-content batch was
    generated by spawning Claude Code subagents directly, not through
    this pipeline), and the realistic dollar savings for a few-hundred-
    request run are modest. If this pipeline gets used again and caching
    becomes worth it: the row-specific context_lines are currently
    ordered BEFORE the shared example/collections/techniques text in
    `user_message` below -- caching only works on a stable *prefix*, so
    that ordering has to flip (shared content first, then a
    cache_control breakpoint, then the row-specific bits) before adding
    cache_control anywhere would actually do anything.
    """
