"""Throwaway: re-ingests both the latte-mistakes query and the Southern
Living query in isolation through the newly-fixed pipeline (many-sub-
question format, content_opportunity extended to manual-submission,
anti-AI-tell style guidance) to see the actual results. Deleted after
use."""
...
def ingest(base: str, auth: tuple[str, str], sender: str, subject: str, text: str) -> None:
    payload = {
...
def main() -> None:
    ...
    ingest(base, auth, "noreply@helpareporter.com", "[RETEST2] HARO latte-mistakes query", LATTE_QUERY)
    ingest(base, auth, "noreply@connectively.us", "[RETEST2] Southern Living query", SOUTHERN_LIVING_QUERY)
