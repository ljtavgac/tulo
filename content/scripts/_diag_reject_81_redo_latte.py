"""Throwaway: rejects prospect id=81 (the raw-digest fallback caused by
the ```json code-fence parsing bug, now fixed) and re-ingests the real
latte query one more time against the fixed pipeline. Deleted after
use."""
...
def main() -> None:
    ...
    r = requests.get(
        ...
        params={"prospect_id": 81, "status": "rejected", "show": "all"},
        auth=auth, timeout=30, allow_redirects=False,
    )
    print(f"reject id=81 -> status={r.status_code}")
