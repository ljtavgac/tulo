"""Throwaway: verifies both bug fixes reported against the live queue.

1. Rejects id=82 (the latte reply drafted under the old prompt, with
   prose justification baked into reporter-facing copy instead of
   placeholder tags) and re-ingests the latte query fresh against the
   fixed prompt, then prints the new reply's body to confirm the
   [URL placeholder -- pending: "..."] / [can't produce a URL for this]
   tags appear instead of prose.
2. Fetches the raw outreach-queue HTML (show=all) and checks structural
   markers to confirm: Southern Living's group renders a neutral
   <div class="group-header"> instead of nesting one card under
   another, and no content_opportunity card shows body-preview text.
...
def main() -> None:
    ...
    r = requests.get(
        ...
        params={"prospect_id": 82, "status": "rejected", "show": "all"},
        auth=auth, timeout=30, allow_redirects=False,
    )
    print(f"reject id=82 -> status={r.status_code}")
    ...
    reply_rows = [row for row in rows if not row.get("proposed_template_type") and "latte" in (row.get("source_query") or "").lower()]
    for row in sorted(reply_rows, key=lambda r: r["id"]):
        print(f"\n--- reply id={row['id']} status={row['status']} ---")
        print(row["body_preview"])

    print("\n=== HTML structural check (show=all) ===")
    r = requests.get(f"{base}/admin/outreach-queue", params={"show": "all"}, auth=auth, timeout=30)
    html = r.text
    group_headers = re.findall(r'<div class="group-header">([^<]*)</div>', html)
    print(f"group-header blocks found: {group_headers}")
    body_preview_texts = re.findall(r'<div class="body-preview">([^<]{0,80})', html)
    placeholder_leaks = [t for t in body_preview_texts if "Placeholder" in t or "placeholder" in t]
    print(f"body-preview blocks containing 'placeholder' (should be empty): {placeholder_leaks}")
